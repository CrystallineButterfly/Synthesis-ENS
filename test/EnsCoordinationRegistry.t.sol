...
import {EnsCoordinationRegistry} from "src/EnsCoordinationRegistry.sol";

contract EnsCoordinationRegistryTest is Test {
    EnsCoordinationRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_ens");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new EnsCoordinationRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        EnsCoordinationRegistry freshController = new EnsCoordinationRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
